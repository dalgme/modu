...
export default async function Page({ searchParams }: { searchParams?: { view?: string } }) {
  ...
  return (
    <main className="flex flex-col gap-5">
      ...
      <CaseBoardTabs basePath="/institution/board" active={view} />
      {view === 'stage' && (
        <StageBoard items={cases} basePath="/institution/cases" editGrantIds={editGrantIds} />
      )}
      {view === 'mentor' && (
        <MentorBoard items={cases} basePath="/institution/cases" editGrantIds={editGrantIds} />
      )}
      {view === 'mentee' && (
        <CaseProgressList
          items={cases}
