...
export async function listDelayedCases(programId: string, supportTypeId?: string | null): Promise<DelayedCase[]> {
  ...
  const [{ data: assigns }, { data: logs }, { data: history }, { data: extraOk }] = await Promise.all([
    admin.from('mentor_assignments').select('case_id, mentor_id, assigned_at').in('case_id', caseIds).eq('is_active', true),
    admin.from('mentoring_logs').select('case_id, started_at, report_registered_at').in('case_id', caseIds),
    admin
  ...
  const roundsByCase = new Map<string, { count: number; lastActivity: string | null }>();
  for (const l of logs ?? []) {
    const cur = roundsByCase.get(l.case_id) ?? { count: 0, lastActivity: null };
    cur.count += 1;
    const act = l.report_registered_at ?? l.started_at;
  ...
  for (const c of list) {
    ...
    const rounds = roundsByCase.get(c.id) ?? { count: 0, lastActivity: null };
    ...
    } else if (c.status === 'mentor_assigned' && rounds.count === 0) {
      const d = daysSince(assign?.assigned_at ?? c.updated_at);
      if (d >= DELAY_DAYS.no_round) out.push({ ...base, kind: 'no_round', days: d });
    } else if (c.status === 'in_progress' && rounds.count < target) {
      const d = daysSince(rounds.lastActivity ?? c.updated_at);
