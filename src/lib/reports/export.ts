...
export function buildReportWorkbook(tab: string, m: ProgramMetrics, cases: CaseListItem[], settlements: SettlementItem[], programName: string): Buffer {
  ...
  if (tab === 'overview' || tab === 'cases') {
    ...
    add('케이스', [
      ...
      ...cases.map((c) => [c.business_name, c.owner_name, c.supportTypeName ?? '', CASE_STATUS_META[c.status].short, c.mentorName ?? '', c.roundsDone, c.requiredRounds, c.created_at.slice(0, 10)]),
    ]);
  }
