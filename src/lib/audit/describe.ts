...
const RULES: { test: (a: string) => boolean; category: string; text: (a: string, m: Record<string, unknown>, l: AuditLike) => string }[] = [
  ...
  { test: (a) => a === 'sms.login_guide', category: '문자', text: () => '로그인 안내 문자를 발송했습니다' },
  { test: (a) => a === 'mentor_form.setting', category: '설정', text: (_a, m) => `위촉 서식(${str(m.form_key) ?? ''}) 설정을 저장했습니다${m.enabled === true ? ' · 사용' : m.enabled === false ? ' · 미사용' : ''}${str(m.method) ? ` · ${str(m.method) === 'file' ? '파일 첨부' : '웹 작성'}` : ''}` },
  { test: (a) => a === 'mentor_form.template', category: '설정', text: (_a, m) => `위촉 서식(${str(m.form_key) ?? ''}) 표준양식 파일을 등록했습니다${str(m.file_name) ? ` (${str(m.file_name)})` : ''}` },
  { test: (a) => a === 'program.feature', category: '플랫폼', text: (_a, m) => `행사 기능(${str(m.key) ?? ''})을 ${m.enabled === true ? '활성화' : '비활성화'}했습니다` },
  { test: (a) => a === 'account.self_register', category: '회원', text: (_a, m) => `확인코드로 운영사 총괄담당자(PL) 계정을 직접 등록했습니다${str(m.email) ? ` (${str(m.email)})` : ''}` },
  { test: (a) => a === 'mentor_form.submit', category: '회원', text: (_a, m) => `멘토가 위촉 서식(${str(m.form_key) ?? ''})을 제출했습니다${str(m.method) === 'file' ? ' (파일)' : ' (웹 작성)'}` },
  { test: (a) => a === 'mentor_form.reject', category: '회원', text: (_a, m) => `위촉 서식(${str(m.form_key) ?? ''}) 제출물을 반려(삭제)했습니다` },
  { test: (a) => a === 'round.report', category: '회차', text: (_a, m) => `${num(m.round_no) ?? ''}회차 보고서(2단계)를 등록했습니다${str(m.report_kind) === 'file' ? ' (파일 업로드)' : ' (웹 작성)'}` },
  { test: (a) => a === 'case.team_info', category: '케이스', text: (_a, m) => `아이템 정보를 저장했습니다${str(m.item) ? ` (${str(m.item)})` : ''}` },
  { test: (a) => a === 'case.team_member_add', category: '케이스', text: (_a, m) => `팀원 '${str(m.name) ?? ''}' 을 추가했습니다` },
  { test: (a) => a === 'case.team_member_update', category: '케이스', text: (_a, m) => `팀원 '${str(m.name) ?? ''}' 정보를 수정했습니다` },
  { test: (a) => a === 'case.team_member_delete', category: '케이스', text: (_a, m) => `팀원 '${str(m.name) ?? ''}' 을 삭제했습니다` },
  ...
  { test: (a) => a === 'case.delete', category: '케이스', text: (_a, m) => `케이스 '${str(m.owner_name) ?? ''}/${str(m.business_name) ?? ''}' 를 완전 삭제했습니다 (회차 ${num(m.rounds) ?? 0} · 정산 ${num(m.settlements) ?? 0} · 파일 ${num(m.files) ?? 0})` },
  ...
  { test: (a) => a === 'batch.confirmed', category: '품의', text: () => '발주처가 정산을 확인했습니다 (열린 정산이 없는 케이스는 종결)' },
  { test: (a) => a === 'batch.paid', category: '품의', text: () => '지급 완료로 기록했습니다' },
  // P31 정산·품의·리포트 추가 규칙
  { test: (a) => a === 'settlement.partial_confirmed', category: '정산', text: (_a, m) => `부분 정산을 확정 저장했습니다${num(m.net) ? ` (실지급 ${won(num(m.net))})` : ''}` },
  { test: (a) => a === 'settlement.rolled_back', category: '정산', text: (_a, m) => `정산 스냅샷을 자동 취소(롤백)했습니다${str(m.reason) ? ` — ${str(m.reason)}` : ''}` },
  { test: (a) => a === 'settlement.withholding_recomputed', category: '정산', text: (_a, m) => (str(m.reason) === 'batch_reverted_to_draft' ? '품의가 작성 중으로 돌아가 원천징수를 원복했습니다' : '품의 단위 과세최저한 초과로 원천징수를 다시 계산했습니다') },
  { test: (a) => a === 'batch.item_added', category: '품의', text: (_a, m) => `정산 건을 품의에 편성했습니다${num(m.net) ? ` (실지급 ${won(num(m.net))})` : ''}` },
  { test: (a) => a === 'batch.item_removed', category: '품의', text: (_a, m) => `정산 건을 품의에서 제외했습니다${num(m.net) ? ` (실지급 ${won(num(m.net))})` : ''}` },
  { test: (a) => a === 'batch.returned', category: '품의', text: (_a, m) => `발주처가 지급 품의를 반려했습니다${str(m.reason) ? ` — ${str(m.reason)}` : ''}` },
  { test: (a) => a === 'batch.meta_updated', category: '품의', text: () => '품의 제목·메모를 수정했습니다' },
  { test: (a) => a === 'report.renamed' || a === 'report.hidden' || a === 'report.unhidden', category: '리포트', text: (a) => (a === 'report.renamed' ? '종합결과리포트 제목을 변경했습니다' : a === 'report.hidden' ? '종합결과리포트를 숨겼습니다' : '종합결과리포트를 복원했습니다') },
  ...
  { test: (a) => a === 'survey.campaign_created', category: '조사', text: (_a, m) => `조사를 개설했습니다 — ${str(m.title) ?? ''} (대상 ${num(m.targets) ?? 0}명 스냅샷)` },
  { test: (a) => a === 'survey.campaign_notified', category: '조사', text: (_a, m) => `조사 ${m.only_unresponded ? '미참여자 독려' : '초대'} 문자를 발송했습니다 (발송 ${num(m.sent) ?? 0} · 실패 ${num(m.failed) ?? 0})` },
  { test: (a) => a === 'survey.campaign_closed', category: '조사', text: () => '조사를 종료했습니다' },
  { test: (a) => a === 'survey.campaign_reopened', category: '조사', text: () => '조사를 다시 열었습니다' },
