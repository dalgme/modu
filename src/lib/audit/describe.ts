...
const RULES: { test: (a: string) => boolean; category: string; text: (a: string, m: Record<string, unknown>, l: AuditLike) => string }[] = [
  ...
  { test: (a) => a === 'mentor_docs.checklist_saved', category: '설정', text: (_a, m) => m.cleared === true ? '멘토 서류 수령 목록의 그룹 별도 설정을 해제했습니다 (행사 공통 따름)' : `멘토 서류 수령 목록을 저장했습니다${m.enabled === true ? ' · 사용' : m.enabled === false ? ' · 미사용' : ''}${typeof m.count === 'number' ? ` · 서류 ${m.count}종` : ''}${str(m.support_type_id) ? ' (그룹 전용)' : ' (행사 공통)'}` },
  ...
  { test: (a) => a === 'mentor_docs.receipt', category: '멘토', text: (_a, m) => `멘토 서류 ${Array.isArray(m.item_keys) ? `${m.item_keys.length}종` : ''} 수령 여부를 ${m.received === true ? '수령' : '미수령'}으로 기록했습니다` },
