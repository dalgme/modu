...
interface StatusBadgeProps {
  status: CaseStatus;
  /** 앞에 단계 번호 표시 (예: "7 · 진흥원 승인") */
  showStep?: boolean;
  className?: string;
}

/**
 * 11단계 케이스 상태를 공통 배지로 표시한다 (설계문서 6.3 · CLAUDE.md 6절).
 * 모든 화면에서 이 컴포넌트로 상태를 표시해 색상·라벨을 통일한다.
 */
export function StatusBadge({ status, showStep = false, className }: StatusBadgeProps) {
