...
const JOURNEY = [
  ...
  },
  {
    icon: NotebookPen,
    title: '컨설팅 회차 등록',
    desc: '회차는 2단계입니다. ① [회차 등록]에서 일자·시간·방법(온/오프라인)·참가자·장소를 클릭으로 남기고(일정 전 사전 등록 가능) → ② 진행 후 그 회차의 [보고서 등록]에서 내용을 웹으로 쓰거나 파일을 올립니다(사진 첨부 가능). 보고서까지 등록돼야 이행으로 인정되고 정산에 포함됩니다. 단가는 등록 시점 값으로 고정됩니다.',
    features: ['1단계 계획은 60일 앞까지 미리 등록, 진행 전에는 일정 수정·삭제 가능', '웹 작성 시 행사/그룹 양식으로 보고서 PDF 자동 생성', '같은 멘티·같은 날 상한, 1일 최대 3건, 시간 겹침 자동 검증', '멘티 확인 서명(정책 시)은 보고서 등록 후 요청'],
