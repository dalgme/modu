-- ============================================================================
-- 0040_consulting_report_attendee_row.sql
-- 컨설팅 결과보고서 상단 정보표를 원본 서식과 정합성 있게 보정.
--  - 컨설팅주제 를 전폭(colspan=3) 단독 행으로 분리
--  - '참석자'(신청업체 대표=멘티) 행 신설 + 참석자 서명 {{{applicant_sign_html}}}
--    · 참석자 이름은 대표자({{owner_name}}) 로 표기, (인) 자리에 서명 이미지 삽입
--  - 컨설턴트(멘토) 서명은 기존과 동일하게 {{{consultant_sign_html}}} 유지
...
update public.document_templates set
  html_content = $html$<!doctype html><html lang="ko"><head><meta charset="utf-8"><style>
    ...
    <table><colgroup><col style="width:18%"><col style="width:32%"><col style="width:18%"><col style="width:32%"></colgroup>
      ...
      <tr><th>컨설팅주제</th><td class="l" colspan="3">{{topic}}</td></tr>
      <tr><th>참석자</th><td class="l">{{owner_name}} {{{applicant_sign_html}}}</td><th>컨설턴트</th><td class="l">{{consultant_name}} {{{consultant_sign_html}}}</td></tr></table>
    <h2>1. 컨설팅</h2>
